...
public class CardContainer : MonoBehaviour
{
    ...
    private void SetCardsPosition()
    {
        ...
        var cardsTotalWidth = cards.Sum(card => Mathf.Min(card.width, 250) * Mathf.Min(card.transform.lossyScale.x, 1.5f));
    ...
    private void DistributeChildrenToFitContainer(float childrenTotalWidth)
    {
        ...
        foreach (CardWrapper child in cards)
        {
            ...
            var adjustedChildWidth = Mathf.Min(child.width, 250) * Mathf.Min(child.transform.lossyScale.x, 1.5f);
